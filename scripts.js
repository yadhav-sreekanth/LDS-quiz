...
const SUPABASE_ANON_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFjdmJxaXp3bWh0dWlucHF0Ynp6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTc1OTgyMjIsImV4cCI6MjA3MzE3NDIyMn0.cSRjn0n1MowMMkILc-WEuotSN2Cj78jpbxJhtEqRjl4';
...
function hasModeratorPower(profile) {
  ...
  return profile?.is_dev || ['developer', 'teacher', 'chairman', 'official'].includes(role);
}
...
function hasMonitoringPower() {
  const inferred =
    Array.isArray(userProfile?.leaderboard_badges) && userProfile.leaderboard_badges[0]
      ? String(userProfile.leaderboard_badges[0]).toLowerCase()
      : '';
  const role = (userProfile?.role_badge || inferred || '').toLowerCase();
  if (!role) return false;
  // Only developer, official, teacher should see Monitoring
  return (
    ['developer', 'official', 'chairman', 'senior_principal', 'teacher'].includes(role) ||
    !!userProfile?.is_dev ||
    (userProfile?.email || '').toLowerCase() === 'yadhavvsreelakam@gmail.com'
  );
}
...
function canStartQuizByRole() {
  ...
  if (
    ...
    return true;
  if (['principal', 'senior_principal', 'teacher', 'chairman', 'official'].includes(role))
    return false;
  return true;
}
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  try {
    await hydrateRolesForCurrentUser();
  } catch (_) {}
  ...
  if (photoInput) {
    ...
    const clickableImgs = [settingsProfilePhotoEl, profilePhotoEl, mobileProfilePhotoEl].filter(
      Boolean
    );
...
function renderUserSummary(profile) {
  ...
  const isDev =
    (profile.email || '').toLowerCase() === 'yadhavvsreelakam@gmail.com' || profile.is_dev;
  ...
  if (roleBadge && roleBadge.toLowerCase() !== 'student') {
    ...
    badgeHtml = `<span class="dev-badge ${badgeClass}">${roleBadge
      .replace('_', ' ')
      .toUpperCase()}</span>`;
  }
...
function hasDevPowers() {
  const emailIsDev =
    userProfile && (userProfile.email || '').toLowerCase() === 'yadhavvsreelakam@gmail.com';
...
function ensureDeveloperVisibleAndInit() {
  const isDevEmail =
    hasDevPowers() && (userProfile?.role_badge || '').toLowerCase() === 'developer';
  ...
  const bind = () => {
    ...
    ideaBtn.onclick = async () => {
      ...
      if (!title || !body) {
        ideaMsg.textContent = 'Title and description required.';
        return;
      }
      const { error } = await supabase
        .from('announcements')
        .insert([{ title: `[Idea] ${title}`, body }]);
      if (error) {
        ideaMsg.textContent = 'Failed: ' + error.message;
        return;
      }
    ...
    addBtn.onclick = () => {
      try {
        ...
        if (!qt || !o1 || !o2 || !o3 || !o4) {
          msg.textContent = 'Fill question and all four options.';
          return;
        }
    ...
    grantBtn.onclick = async () => {
      ...
      if (!query) {
        powerMsg.textContent = 'Enter a name or user id.';
        return;
      }
      ...
      try {
        ...
        let rpcErr = null;
        try {
          await supabase.rpc('grant_roles_sync', {
            p_user_id: target.id,
            p_roles: ['dev', 'developer'].includes(role) ? ['dev'] : [role]
          });
        } catch (e) {
          rpcErr = e;
        }
        if (rpcErr) {
          const { error } = await supabase
            ...
            .eq('id', target.id);
          if (error) {
            powerMsg.textContent = 'Failed to grant: ' + error.message;
            return;
          }
        }
    ...
    revokeBtn.onclick = async () => {
      ...
      try {
        ...
        let rpcErr = null;
        try {
          await supabase.rpc('revoke_roles_sync', { p_user_id: target.id });
        } catch (e) {
          rpcErr = e;
        }
        if (rpcErr) {
          const { error } = await supabase
            ...
            .eq('id', target.id);
          if (error) {
            powerMsg.textContent = 'Failed to revoke: ' + error.message;
            return;
          }
        }
...
async function devSaveQuestionsToStorage(text) {
  try {
    ...
    const { data, error } = await supabase.storage
      .from('config')
      .upload('questions.json', blob, { upsert: true, contentType: 'application/json' });
    if (error) throw error;
    return true;
  } catch (e) {
    console.warn('save questions failed', e);
    // bubble up readable message so UI can show it
    return e && (e.message || e.error || e.statusText)
      ? e.message || e.error || e.statusText
      : false;
  }
}
...
async function startQuiz() {
  ...
  quizQuestions = shuffleArray(pickFrom)
...
function finalizeAttempt() {
  ...
  try {
    ...
    console.warn('award check failed', e);
  }
...
async function loadPosts() {
  ...
  const { data: posts, error } = await supabase
    .from('posts')
    .select(
      'id, user_id, content, image_path, created_at, is_pinned, users:users!posts_user_id_fkey(full_name, profile_photo, is_dev, role_badge, leaderboard_badges)'
    )
  ...
  posts.forEach((post) => {
    ...
    if (role) {
      nameWithBadge += ` <span class="dev-badge ${role.toLowerCase()}">${role
        .replace('_', ' ')
        .toUpperCase()}</span>`;
    ...
    el.innerHTML = `
                        <div class="post-head">
                            <img src="${
                              (post.users && post.users.profile_photo) || 'logonew.png'
                            }" />
                        ...
                        <div class="post-body">
                            ...
                            <div class="post-actions">
                                <button class="like-btn" data-post-id="${
                                  post.id
                                }" aria-pressed="false"><i class="fas fa-heart"></i> <span class="like-text">Like</span> <span class="like-count" aria-live="polite">0</span></button>
                                <button class="comment-btn" data-post-id="${
                                  post.id
                                }"><i class="fas fa-comment"></i> Comment</button>
                                <button class="share-btn" data-post-id="${
                                  post.id
                                }"><i class="fas fa-share"></i> Share</button>
                                ${
                                  isDevEmail
                                    ? `<button class="pin-btn" data-post-id="${
                                        post.id
                                      }" data-pinned="${
                                ...
                                ${
                                  post.user_id === userId || isDevEmail
                              ...
                              post.id
                                ...
                                <div class="comment-input">
                                    <div class="emoji-picker">
                                        <button type="button" class="emoji-btn" data-post-id="${
                                          post.id
                                        }">😀</button>
                                        <button type="button" class="emoji-btn" data-post-id="${
                                          post.id
                                    ...
                                    <div class="comment-form">
                                        <input type="text" class="comment-text" placeholder="Write a comment..." data-post-id="${
                                          post.id
                                        }">
...
async function loadComments(postId, commentsListEl) {
  try {
    const { data: comments, error } = await supabase
      .from('post_comments')
      .select(
        'id, content, created_at, user_id, is_pinned, users:users!post_comments_user_id_fkey(full_name, profile_photo, is_dev, role_badge)'
      )
      .eq('post_id', postId)
    ...
    if (comments.length === 0) {
      commentsListEl.innerHTML =
        '<p style="color: #6b7280; font-style: italic; padding: 1rem;">No comments yet. Be the first to comment!</p>';
      return;
    }
...
async function hydrateAndBindLikes(container) {
  try {
    const postIds = Array.from(container.querySelectorAll('.like-btn')).map((b) =>
      b.getAttribute('data-post-id')
    );
  ...
  try {
    const { data: likesAll } = await supabase
      ...
      .in(
        'post_id',
        Array.from(container.querySelectorAll('.like-btn')).map((b) =>
          b.getAttribute('data-post-id')
        )
      );
    const countMap = {};
...
async function deleteAccount() {
  ...
  try {
    ...
    if (postIds.length > 0) {
      const { error } = await supabase.from('post_likes').delete().in('post_id', postIds);
    ...
    // 1d) Awards
    {
      const { error } = await supabase.from('awards').delete().eq('user_id', userId);
    ...
    let attemptIds = [];
    {
      const { data, error } = await supabase.from('attempts').select('id').eq('user_id', userId);
    ...
    if (attemptIds.length > 0) {
      ...
      const { error: atErr } = await supabase.from('attempts').delete().in('id', attemptIds);
    ...
    let quizIds = [];
    {
      const { data, error } = await supabase.from('quizzes').select('id').eq('user_id', userId);
    ...
    if (quizIds.length > 0) {
      ...
      const { error: qzErr } = await supabase.from('quizzes').delete().in('id', quizIds);
...
async function deleteUserStorageAssets(uid) {
  await Promise.all([deleteFolder('avatars', `${uid}`), deleteFolder('post-images', `${uid}`)]);
}
...
async function loadAccount() {
  try {
    ...
    document.getElementById('accountName').innerHTML = escapeHtml(
      userProfile.full_name || 'Student'
    );
    document.getElementById('accountEmail').textContent = userProfile.email || '';
    // Show total points in My Space (not weekly)
    document.getElementById('accountPoints').textContent = `Total Points: ${
    ...
    if (Array.isArray(userProfile.leaderboard_badges)) {
      userProfile.leaderboard_badges.forEach((b) => {
        if (b && roles.length < 2) roles.push(b);
      });
    }
    roles.forEach((r) =>
      badges.push({
        name: String(r).replace('_', ' ').toUpperCase(),
        color: '#eef2ff',
        textColor: '#1a3d7c',
        solid: false
      })
    ...
    badges.forEach((b) => {
      ...
      tag.style.cssText = `display:inline-block;padding:.3rem .6rem;border-radius:999px;background:${
        b.color
      };color:${
        b.textColor || '#111'
      };font-weight:600;font-size:.8rem;border:1px solid rgba(0,0,0,.08)`;
    ...
    try {
      ...
      if (awards && awards.length) {
        ...
        awards.forEach((a) => {
          ...
          chip.style.cssText =
            'display:inline-block;margin:.25rem .25rem 0 0;padding:.25rem .55rem;border-radius:999px;background:#eef2ff;color:#1a3d7c;font-weight:600;font-size:.75rem;border:1px solid #dbe3ff';
    ...
    function renderConnectionCard(u, isFollowing) {
      ...
      card.innerHTML = `
                            <img src="${
                              u.profile_photo || 'https://via.placeholder.com/48'
                            }" alt="${u.full_name}" class="connection-avatar">
                            ...
                            <div class="connection-actions">
                                ...
                                }" data-user-id="${u.id}" data-follow="${isFollowing ? '1' : '0'}">
                                    ${isFollowing ? 'Unfollow' : 'Follow'}
                                </button>
                            </div>
                        `;
    ...
    followerUsers.forEach((u) =>
      followersList.appendChild(renderConnectionCard(u, followingIdList.includes(u.id)))
    );
...
function bindAdminManageUsers() {
      ...
      (users || [])
        ...
        .forEach((u) => {
          ...
          row.onclick = () => {
            ...
            document.querySelectorAll('.admin-user-row').forEach((r) => (r.style.background = ''));
  ...
  document.getElementById('adminAddFollower')?.addEventListener('click', async () => {
    ...
    const { error } = await supabase
      .from('follows')
      .insert([{ follower_id: fid, followed_id: selectedId }]);
